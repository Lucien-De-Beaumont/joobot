...
module.exports = {
    ...
    async runInteraction(client, interaction) {
        ...
        const [results0] = await db.query(`SELECT * FROM webhook WHERE discordid=${db.escape(interaction.guild.members.cache.find(member => member.displayAvatarURL() == message.embeds[0].thumbnail.url).id)} ORDER BY nom ASC LIMIT 5 OFFSET ${(numberPages * 5)}`);
