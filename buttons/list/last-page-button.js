...
module.exports = {
    ...
    async runInteraction(client, interaction) {
        ...
        let maxNumberPages = parseInt(message.embeds[0].footer.text.slice(message.embeds[0].footer.text.indexOf('sur ') + 4))
            ...
            numberPages = maxNumberPages

        const [results0] = await db.query(`SELECT * FROM Icon99 WHERE discordid=${db.escape(interaction.guild.members.cache.find(member => member.displayAvatarURL() == message.embeds[0].thumbnail.url).id)} AND isbot = 'FALSE' ORDER BY nom ASC LIMIT 5 OFFSET ${((numberPages - 1) * 5)}`);
        ...
        const listEmbed = new Discord.MessageEmbed()
            ...
            .setFooter({ text: `Page ${+numberPages} sur ${+maxNumberPages}` })
            .setTimestamp()
