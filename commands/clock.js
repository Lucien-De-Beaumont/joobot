...
module.exports = {
    ...
    runInteraction(client, interaction) {
        try { eval('config.guild_' + interaction.guild.id + ".channels['meteo']") ; eval('config.guild_' + interaction.guild.id + ".channels['clock']") } catch (err) { return Logger.debug('fatal error occured:' + err) }
        ...
        client.channels.cache.get(eval('config.guild_' + interaction.guild.id + '.channels[\'clock\']')).send({ embeds: [embed] })
    }
}